...
class MainActivity: AppCompatActivity() {
    ...
    private fun login() {
        ...
        thread {
            try {
                ...
                api.setToken(token)
                runOnUiThread { showApp("Đăng nhập thành công: $name"); sync(); startFastPolling(); startOrderAlertService() }
            } catch (e: Exception) {
    ...
    /** Bật cảnh báo đơn mới chạy nền bằng Foreground Service. */
    private fun startOrderAlertService() {
        val intent = Intent(this, OrderAlertService::class.java)
        runCatching {
    ...
    /** Dừng cảnh báo nền khi shipper đăng xuất. */
    private fun stopOrderAlertService() {
        runCatching { stopService(Intent(this, OrderAlertService::class.java)) }
    }
}
